...
| 3 | Rewrite `db.rs` | ✅ Implemented (uncommitted) | With `note_meta` table, `get_metadata` stub, `total_notes`→`total_files` |
| 4 | `chunker.rs` | ✅ Implemented (uncommitted) | Header/paragraph splitter |
| 5 | `embedder.rs` | ✅ Implemented (uncommitted) | ONNX pipeline stub (`Embedder::load` returns Err without model) |
| 6 | Rewrite `search.rs` | ❌ Not started | Still uses old `db.search()` stub |
| 7 | Rewrite `indexer.rs` | ⚠️ Partial (uncommitted) | `flush_batch` uses new API; `note_meta` table added; tests broken (`total_notes` → `total_files`) |
| 8 | CLI `dive --mode` | ❌ Not started | |
| 9 | CLI `dredge` + `setup` | ❌ Not started | |
| 10 | CLI `chart` + `scan` | ❌ Not started | |

**Blocking issues:**
1. `indexer.rs` tests use `total_notes` field (renamed to `total_files` in `VaultStats`)
2. `cli/src/commands/log.rs` calls non-existent `db.list_all_metadata()` (only `get_metadata` stub exists)
3. `cli/src/commands/tide.rs` uses `stats.total_notes` → should be `total_files`
4. `search.rs` still has old `todo!()` / stub — needs full rewrite

**Next immediate step:** Fix indexer tests (`total_notes` → `total_files`), then rewrite `search.rs` (Task 6) to unblock all downstream work.
...
## Task 3: Rewrite `db.rs` with new schema and migration ⚠️ (implemented, uncommitted — `note_meta` table added beyond plan)
...
- [ ] **Step 6: Commit (pending — search.rs, indexer.rs, CLI also need to be fixed first)**
...
## Task 4: Implement `chunker.rs` ✅ (implemented, uncommitted)
...
## Task 5: Implement `embedder.rs` (ONNX pipeline) ✅ (implemented, uncommitted)
...
## Task 6: Rewrite `search.rs` with FTS / vec / hybrid modes ❌ (not started)
...
## Task 7: Rewrite `indexer.rs` ⚠️ (partial — only `total_notes`→`total_files` fix done in uncommitted diff; full chunk-based rewrite NOT yet implemented)
...
(Implementation details unchanged — see above. Note: `flush_batch` references `tokenizer` variable that is not in scope — this is a known issue in the plan's code.)
