...
VLM API 抽出結果も `file_cache.vlm_hash` カラムにキャッシュされます。再インデックス時に PDF バイナリが変更されていない場合は、VLM API 呼び出しがスキップされます。

```
PDF バイナリ SHA-256 → file_cache.vlm_hash と比較
    ├── 一致 → VLM API 呼び出しスキップ（既存チャンクから再構成）
    └── 不一致 → VLM API 呼び出し → 結果をインデックス + ハッシュ保存
```

これにより、再インデックス時の不要な VLM API コストを削減します。
