...
pub fn call_tool(
    ...
    match name {
        "search_local_notes" => {
            ...
            // Apply sensitive data masking on the final output (after snippet extraction)
        ...
        "get_surrounding_context" => {
            ...
            // Apply sensitive data masking on the final output
            let masked = shiotsuchi_core::sensitive::mask_sensitive_data(&out, sensitive_config);
            Ok(json!({
                "content": [{"type": "text", "text": masked}]
            }))
        }
...
mod tests {
    ...
    fn test_search_local_notes_rejects_nonexistent_vault_dir() {
        ...
        let result = call_tool("search_local_notes", &args, &vaults, &db_path, None);
    ...
    fn test_search_local_notes_vec_without_embedder_returns_message() {
        ...
        let result = call_tool("search_local_notes", &args, &vaults, &db_path, None).unwrap();
    ...
    fn test_index_status_returns_counts() {
        ...
        let vaults = vec![("default".to_string(), temp.path().to_path_buf())];
        let Some(db_path) = setup_db(&temp) else { return; };
        let result = call_tool("index_status", &serde_json::json!({}), &vaults, &db_path, None).unwrap();
        let text = result["content"][0]["text"].as_str().unwrap();
    ...
    fn test_get_surrounding_context_returns_chunks() {
        ...
        let result = call_tool("get_surrounding_context", &args, &vaults, &db_path, None);
    ...
    fn test_unknown_tool_returns_error() {
        ...
        let result = call_tool("nonexistent_tool", &serde_json::json!({}), &vaults, &db_path, None);
        assert!(result.is_err());
    }
