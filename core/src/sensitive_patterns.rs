...
pub fn get_builtin_patterns() -> Vec<&'static str> {
    vec![
        // PII - Email addresses
        ...
        r"(?i)(?:aws|secret|access).{0,20}[A-Za-z0-9/+=]{40}",
        ...
        r"-----BEGIN (?:RSA |EC |DSA )?PRIVATE KEY-----(?s:.+)-----END (?:RSA |EC |DSA )?PRIVATE KEY-----",
        ...
        r"\+[\d\-\.\s\(\)]{7,20}|\d{3,4}[-.\s]\d{3,4}[-.\s]\d{4}",
    ]
}

/// Get the placeholder name for a pattern index
pub fn get_placeholder(index: usize) -> &'static str {
    ...
    PLACEHOLDERS.get(index).unwrap_or(&"REDACTED")
}
...
mod tests {
    ...
    fn test_email_pattern_matches() {
        let pattern = get_builtin_patterns()[0];
    ...
    fn test_openai_key_pattern() {
        let pattern = get_builtin_patterns()[1];
        let re = regex::Regex::new(pattern).unwrap();
    ...
    fn test_anthropic_key_pattern() {
        let pattern = get_builtin_patterns()[2];
        let re = regex::Regex::new(pattern).unwrap();
    ...
    fn test_github_token_pattern() {
        let pattern = get_builtin_patterns()[3];
        let re = regex::Regex::new(pattern).unwrap();
    ...
    fn test_gitlab_token_pattern() {
        let pattern = get_builtin_patterns()[4];
        let re = regex::Regex::new(pattern).unwrap();
    ...
    fn test_aws_key_id_pattern() {
        let pattern = get_builtin_patterns()[5];
        let re = regex::Regex::new(pattern).unwrap();
    ...
    fn test_slack_token_pattern() {
        let pattern = get_builtin_patterns()[7];
        let re = regex::Regex::new(pattern).unwrap();
    ...
    fn test_gcp_api_key_pattern() {
        let pattern = get_builtin_patterns()[9];
        let re = regex::Regex::new(pattern).unwrap();
    ...
    #[test]
    fn test_jwt_pattern() {
        let pattern = get_builtin_patterns()[12];
        let re = regex::Regex::new(pattern).unwrap();
        assert!(re.is_match(
    ...
    fn test_phone_pattern() {
        let pattern = get_builtin_patterns()[14];
        let re = regex::Regex::new(pattern).unwrap();
