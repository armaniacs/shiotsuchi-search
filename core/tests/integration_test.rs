...
fn test_end_to_end_index_and_search() {
    ...
    // FTS search
    let search_results = search(&db, &tokenizer, "search engine", 10, SearchMode::Fts, None, None, None, None, None, &[], &HashMap::new(), false).unwrap();

    let ja_results = search(&db, &tokenizer, "形態素", 10, SearchMode::Fts, None, None, None, None, None, &[], &HashMap::new(), false).unwrap();
