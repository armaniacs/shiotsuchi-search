...
## 受け入れ基準
- [x] 各チェックで問題検出時に `[y/N]` プロンプトが表示される（修復可能な問題のみ）
- [x] ユーザーが y を選ぶと修復が実行され結果が表示される
- [x] ユーザーが n を選ぶとスキップされ次のチェックに進む
- [x] 修復前には必ずバックアップが作成される
- [x] Vault ディレクトリ不在は修復を提案せずヒントのみ表示する
- [x] Tokenizer/Embedder 未利用はヒントのみ表示する
- [x] すべての修復が完了した（または不要だった）場合、"All checks passed." と表示される
- [x] 修復に失敗した場合でも後続のチェックは継続される
...
## Definition of Done
- [x] 全 BDD シナリオが自動テストとして実装されパスする（手動確認のシナリオは除く）
- [x] テストカバレッジが基準を満たす（config fix の単体テスト + DB 再インデックスの統合テスト）
- [x] コードレビュー完了
- [x] リファクタリング完了（グリーン後）
- [x] `docs/superpowers/specs/2026-05-25-doctor-fix-design.md` の内容と実装が一致していること

## 完了
2026-05-25 実装・テスト・レビュー完了。`main` ブランチにマージ済み（v0.4.8）。
