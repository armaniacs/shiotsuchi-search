...
## 受け入れ基準
- [x] `notes_meta` テーブルに `file_size INTEGER` と `mtime INTEGER` カラムが追加される
- [x] スキャン時に mtime + size が一致するファイルは SHA-256 計算をスキップする
- [x] mtime + size が変化したファイルのみ SHA-256 再計算 → 差分があればトークナイズ・DB更新
- [x] 旧スキーマからの自動マイグレーションが動作する
- [x] 既存テストがすべてパスする
