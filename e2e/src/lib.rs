...
mod tests {
    ...
    fn shiotsuchi_bin() -> std::path::PathBuf {
        // When running as part of the `shiotsuchi` package, Cargo sets CARGO_BIN_EXE_shiotsuchi at compile time.
        // When running from a separate e2e package, we locate the binary relative to the workspace root.
        if let Ok(path) = std::env::var("CARGO_BIN_EXE_shiotsuchi") {
            return std::path::PathBuf::from(path);
        }
        // Fallback: look in target/debug or target/release relative to workspace root.
        let manifest_dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR"));
        let workspace_root = manifest_dir.parent().unwrap();
        let debug = workspace_root.join("target/debug/shiotsuchi");
        if debug.exists() {
            return debug;
        }
                ...
                if let Ok(event) = rx.recv_timeout(Duration::from_millis(50)) {
                    ...
                    if matches!(
                        ...
                        for path in &event.paths {
                            if let Ok(rel) = path.strip_prefix(&config.vaults[0].1) {
